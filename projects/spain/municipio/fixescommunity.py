...
#
# make fixes pouinted out by community - check pl:Wikipedysta:Magul/Orduña
#
...
# check if page already exists
	...
	if pl_page.exists():
		page_history = pl_page.getVersionHistory()
		only_bot = True
		for i in page_history:
			if i[2] != u'MagulBot':
				only_bot = False
		if page_history[-1][2] != u'MagulBot':
			only_bot = True
		if not only_bot:
			print pl_page.title(), ' ===> ', pl_page.getVersionHistory()[-1]
			webbrowser.open('https://pl.wikipedia.org/w/index.php?title='+ pl_page.title() + u'&action=history')

# be carefull about redirects
