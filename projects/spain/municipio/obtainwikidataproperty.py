...
con = lite.connect('municipio.sqlite')
cur = con.cursor()
cur.execute(u'SELECT wikidata, coa, flag, commons, image, es_page FROM municipio')
data = cur.fetchall()
...
# coat of arms
		if claim['m'][1] == 94:
			if claim['m'][3].replace('_',' ') != coa.replace('_',' ') and coa != '':
				print (('COA: ' + es_page + ' (' + wd + ') =>' + coa + '  ' + claim['m'][3]).encode('utf-8'))
			else:
				sql_upd += "coa='{{#property:P94}}', "
# flag
		elif claim['m'][1] == 41:
			if claim['m'][3].replace('_',' ') != flag.replace('_',' ') and flag != '':
				print (('FLAG: ' + es_page +  ' (' + wd + ') =>' + flag  + ' != ' + claim['m'][3]).encode('utf-8'))
			else:
				sql_upd += "flag='{{#property:P41}}', "
# commons category
		elif claim['m'][1] == 373:
			if claim['m'][3].replace('_',' ') != commons.replace('_',' ') and commons != '':
				if commons.find(u'Category') == 0 and claim['m'][3].replace('_',' ') != commons[9:].replace('_', ' '):
...
# image
			...
			if claim['m'][3].replace('_',' ') != image.replace('_',' ') and image != '':
