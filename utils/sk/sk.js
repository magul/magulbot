...
wp_sk.sz_redirs_tab = {
	...
	'animanga infobox/header' : 'Animanga infobox/Nag³ówek',
	'animanga infobox/header2' : 'Animanga infobox/Nag³ówek2',
	'bez zmian' : 'Stagnacja',
	'bezpodpisu' : 'Podpisuj',
	...
	'mediavideo' : 'Wideo',
	'middot' : '.',
	'mistrzowie olimpijscy w pi³ce no¿nej' : 'Mistrzowie olimpijscy w pi³ce no¿nej mê¿czyzn',
	'multilang' : 'Lang',
	'nor' : 'Twórczo¶æ w³asna',
	...
	'rpr' : 'Reprezentant pi³ki rêcznej',
	...
	'wikisource' : 'Wiki¼ród³a',
	'wikisource-cat' : 'Wiki¼ród³a kat',
	'wikisource-krotki' : 'Wiki¼ród³a krótki',
	'wikisource autor' : 'Wiki¼ród³a autor',
	'witaj-en' : 'Welcome',
	'witaj-fr' : 'Bienvenue',
	'witajip' : 'Anonim',
	'zasada w jednym zdaniu' : 'W skrócie',
	'zdjêcie tyg' : 'Ilustracja na medal',
	'zzw' : 'Zgodnie z Wikietykiet±',
	'\\w' : '\\wrap',
	'.w' : '.wrap',
	'¶wiêta infobox' : '¦wiêty infobox',
	'.w' : '.wrap',
	'.w' : '.wrap'
};
...
/* ------------------------------------------------------------------------ *\
	...
	* Wikipedysta:ABach - za zebranie i opracowanie d³ugiej listy elementów do sprz±tania
	* Wikipedysta:Malarz pl - za gar¶æ kolejnych elementów do sprz±tania
	* Wikipedysta:BartekChom - za pomys³y i gotowe wyra¿enia regularne
	* Wikipedysta:Gregul - za gar¶æ wyra¿eñ regularnych
	* Wikipedysta:PMG - za wytrwa³e i szczegó³owe testowanie
	* Wikipedysta:ToSter - za testy i pomys³y na nowe rozwi±zania
	* Wikipedysta:Beau - za inspiracje i poprawki
\* ------------------------------------------------------------------------ */

//
// Modu³y zewnêtrzne dla projektów siostrzanych
//
...
if (window.wp_sk)
{
	alert('B³±d krytyczny - konflikt nazw!\n\nJeden ze skryptów u¿ywa ju¿ nazwy wp_sk jako zmienn± globaln±.');
}
...
/* =====================================================
	...
	po wci¶niêciu odpowiedniego przycisku
   ===================================================== */
wp_sk.warning = function() {
	...
	if ( this.nochanges ) {
		...
		if ( text.indexOf( summary1 ) > -1 || text.indexOf( summary2 ) > -1 ) {
			// opis ju¿ jest, nie potrzeba nastêpnego
			return;
		}
...
/* =====================================================
	...
	G³ówna funkcja inicjuj±ca i wywo³uj±ca funkcjê
	czyszcz±c±
   ===================================================== */
wp_sk.cleanup = function (input)
{
	...
	//
	// Pobierz zaznaczony fragment (ca³o¶æ je¶li nic nie zaznaczone)
	//
...
/* =====================================================
	...
	Funkcja czyszcz±ca podany na wej¶ciu ci±g znaków str.
	Zwraca przetworzony ci±g znaków.
   ===================================================== */
wp_sk.cleaner = function (str)
{
	...
	//
	// koñcowe porz±dkowanie miêdzywiki itp
...
wp_sk.cleanerLinks = function (str)
{
	...
	// usuniêcie klucza sortowania kat. je¶li w ca³o¶ci jest prefiksem nazwy artyku³u lub nazw± artyku³u
	...
	*/
	...
	//str = str.replace(/\[\[([^|\]])([^|\]]*)\|([^\]])\2([a-zA-Z¿ó³æê¶±¼ñ¯Ó£ÆÊ¦¡¬Ñ]*)\]\]/g, function (a, w1_1, w_rest, w2_1, poza)
	str = str.replace(/\[\[([^|\]])([^|\]]*)\|([^\]])\2([a-z¿ó³æê¶±¼ñ]*)\]\]/g, function (a, w1_1, w_rest, w2_1, poza)
	{
		return (w1_1.toUpperCase()==w2_1.toUpperCase()) ? '[['+w2_1+w_rest+']]'+poza : a;
	});
	// [[Link|link]]er > [[Link|linker]]
	//str = str.replace(/\[\[([^|\]]+)\|([^|\]]+)\]\]([a-zA-Z¿ó³æê¶±¼ñ¯Ó£ÆÊ¦¡¬Ñ]+)/g, '[[$1|$2$3]]');
	str = str.replace(/\[\[([^|\]]+)\|([^|\[\]]+)\]\]([a-z¿ó³æê¶±¼ñ]+)/g, '[[$1|$2$3]]');
	...
	str = str.replace(/([^ \t\n])\[\[([^\]\|:]+)\| +/g, '$1 [[$2|');
	str = str.replace(/\[\[([^\]\|:]+)\| +/g, '[[$1|');
	str = str.replace(/([^ \|]) +\]\]([^ \t\na-zA-Z¿ó³æê¶±¼ñ¯Ó£ÆÊ¦¡¬Ñ])/g, '$1]] $2');
	str = str.replace(/([^ \|]) +\]\]([^a-zA-Z¿ó³æê¶±¼ñ¯Ó£ÆÊ¦¡¬Ñ])/g, '$1]]$2');
	...
	str = str.replace(/\[\[ang\.\]\]/g, '[[jêzyk angielski|ang.]]');
	str = str.replace(/\[\[cz\.\]\]/g, '[[jêzyk czeski|cz.]]');
	str = str.replace(/\[\[fr\.\]\]/g, '[[jêzyk francuski|fr.]]');
	str = str.replace(/\[\[³ac\.\]\]/g, '[[³acina|³ac.]]');
	str = str.replace(/\[\[niem\.\]\]/g, '[[jêzyk niemiecki|niem.]]');
	str = str.replace(/\[\[pol\.\]\]/g, '[[jêzyk polski|pol.]]');
	str = str.replace(/\[\[pl\.\]\]/g, '[[jêzyk polski|pol.]]');
	str = str.replace(/\[\[ros\.\]\]/g, '[[jêzyk rosyjski|ros.]]');
...
/* =====================================================
	...
	Sprz±tanie szablonów
   ===================================================== */
wp_sk.cleanerTpls = function (str)
{
	// niepotrzebna przestrzeñ
	str = str.replace(/\{\{ *([Tt]emplate|[Ss]zablon|msg) *: */g, '{{');

	// zbêdne spacje w szablonach jedno wierszowych
	str = str.replace(/\{\{[ \t]+([^\n\{\} ]+)[ \t]*\}\}/g, '{{$1}}').replace(/\{\{([^\n\{\}]+)[ \t]+\}\}/g, '{{$1}}');
	...
	// wci±ganie {{lang}} do szablonów cytowania
	str = str.replace(/{{(cytuj [^{}]+?)}} {{lang\|([a-z-]+)}}/gi, '{{$1 | jêzyk = $2}}');

	// ujednolicanie nazw szablonów (tabela poni¿ej)
	...
	// poprawka, bo FF wywala siê na czo³gach np. http://pl.wikipedia.org/w/index.php?title=T-72&diff=14511491&oldid=14437344
...
/* =====================================================
	...
	Sprz±tanie pozosta³ych elementów wikisk³adni
   ===================================================== */
wp_sk.cleanerWikiVaria = function (str)
{
	if ( mw.config.get( 'wp-sk-fix-wikipedia-sections', true ) ) {
		// unifikacja nag³ówkowa
		str = str.replace(/[ \n\t]*\n'''? *(Zobacz|Patrz) (te¿|tak¿e|równie¿):* *'''?[ \t]*\n[ \t\n]*/gi, '\n\n== Zobacz te¿ ==\n');
		str = str.replace(/[ \n\t]*\n'''? *(Zobacz|Patrz) (te¿|tak¿e|równie¿):* *'''?[ \t]*(.+)/gi, function(a, w1, w2, linki)
		{
			if (linki.indexOf('[')!=-1)
			{
				// add first list el.
		...
		str = str.replace(/[ \n\t]*\n(=+) *(Zobacz|Patrz) (te¿|tak¿e|równie¿):* *=+[ \n\t]*/gi, '\n\n$1 Zobacz te¿ $1\n');
		str = str.replace(/[ \n\t]*\n'''? *((Zewnêtrzn[ey] )?(Linki?|£±cza|Stron[ay]|Zobacz w (internecie|sieci))( zewn[eê]trzn[aey])?):* *'''?[ \n\t]*/gi, '\n\n== Linki zewnêtrzne ==\n');
		str = str.replace(/[ \n\t]*\n(=+) *((Zewnêtrzn[ey] )?(Linki?|£±cza|Stron[ay]|Zobacz w (internecie|sieci))( zewn[eê]trzn[aey])?):* *=+[ \n\t]*/gi, '\n\n$1 Linki zewnêtrzne $1\n');
		str = str.replace(/[ \n\t]*\n(=+) *([¬¯Z]r[óo]d[³l]a):* *=+[ \n\t]*/gi, '\n\n$1 ¬ród³a $1\n');
	}
	...
	if ( mw.config.get( 'wp-sk-fix-wikipedia-sections', true ) ) {
		...
		str = str.replace(/\n== Przypisy ==[ \t\n]+<references ?\/>/g, '\n{{Przypisy}}');
		str = str.replace(/\n(={3,}) Przypisy \1[ \t\n]+<references ?\/>/g, '\n{{Przypisy|stopieñ= $1}}');
		str = str.replace(/\{\{Przypisy\|stopieñ==/g, '{{Przypisy|stopieñ= =');
	}

	// przypisy - przyprz±tniêcia
	/*
	...
	*/
	...
	str = str.replace(/([a-zA-Z±æê³ñó¶¼¿¡ÆÊ£ÑÓ¦¬¯]{5}|[()\[\]{}".'>])[.]((?:(?:<ref[\s\S]+?(?:<\/ref|\/)>)|(\{\{[Rr]\|[^}]+\}\}))+)/g, "$1$2.");	// d³ugi wyraz lub znak specjalny
	str = str.replace(/([a-zA-Z±æê³ñó¶¼¿¡ÆÊ£ÑÓ¦¬¯][aeiouy±ê])[.]([']*(?:(?:<ref[\s\S]+?(?:<\/ref|\/)>)|(\{\{[Rr]\|[^}]+\}\}))+)/g, "$1$2.");	// krótki z samog³osk±
	...
	// nie dzia³a dla wywo³añ z parametrami grupaN=
	...
	*/
	...
	str = str.replace(/([a-zA-Z±æê³ñó¶¼¿¡ÆÊ£ÑÓ¦¬¯]{5}|[()\[\]{}".'>])[.](\{\{[Ff]akt(?:\|data=[0-9\-]+)?\}\})/g, "$1$2.");	// d³ugi wyraz lub znak specjalny
	str = str.replace(/([a-zA-Z±æê³ñó¶¼¿¡ÆÊ£ÑÓ¦¬¯][aeiouy±ê])[.](\{\{[Ff]akt(?:\|data=[0-9\-]+)?\}\})/g, "$1$2.");	// krótki z samog³osk±
...
{
	// usuwanie unikodowych znaków steruj±cych
	str = str.replace(/[\u200B\uFEFF\u200E]/g, '');

	// korekty dat
	// wystêpuje w interwiki (hr)
	...
	// poprawkowate ró¿ne (kolejno¶æ jest istotna!)
	...
	str = str.replace(/(gmina wiejska w powiecie [a-zA-Z±æê³ñó¶¼¿¡ÆÊ£ÑÓ¦¬¯\-]+ województwa [a-zA-Z±æê³ñó¶¼¿¡ÆÊ£ÑÓ¦¬¯\-]+) II Rzeczpospolitej/g, '\1 II Rzeczypospolitej');
...
/* =====================================================
	...
	Sprz±tanie koñcowe magicznych linków i elementów
	powi±zanych - miêdzywiki, medale dla nich i kategorie.
   ===================================================== */
...
/* =====================================================
	...
	.hide(str)
		ukrywanie tagów specjalnych wraz z ich wnêtrzami
	.show(str)
		przywrócenie ukrytych tagów
   ===================================================== */
//
...
wp_sk.nowiki.hide = function(str)
{
	...
	// póki znaleziono tag otwieraj±cy
	for (var t_i = 0; (m=re.exec(str))!=null; t_i++)
	{
		...
		// szukanie od startu, bo czê¶æ znaków ju¿ usuniêto
		re.lastIndex = start;
	}
...
/* =====================================================
	...
	.arr_i
		indeks pomocniczy, a tak¿e liczba elementów w arr
   ===================================================== */
// object init
...
function PolishCollator() {
	...
	var orderArray = ['aä', '±', '.', 'á', 'b', 'c', 'æ', 'd', 'e', 'ê', 'f', 'g', 'h', 'i', 'j', 'k', 'l', '³', 'm', 'n', 'oö', 'ó', 'p', 'q', 'r', 's', '¶', 't', 'uü', 'v', 'w', 'x', 'y', 'z', '¼', '¿'];
	var orderHash = {};
...
wp_sk.cat.output = function (a)
{
	...
	//
	// sortowanie (je¶li dostêpna odpowiednia funkcja)
	...
	if (reDefSort!="") // je¶li jest jaki¶ defaultsort
	{
		...
		for (var i=0; i<wp_sk.cat.arr_i; i++)
		{
			// je¶li nie by³o defaultsort i puste, to dodajemy domy¶lne, ¿eby nie psuæ
...
wp_sk.cat.getDefSort = function ()
{
	...
	// szukanie nowego je¶li liczba kategorii jest wiêksza od 1
	...
	{
		// zamiana na regexp (¿eby unikn±æ czê¶ciowych dopasowañ)
...
/* =====================================================
	...
	.output(a)
		...
		parametr a jest nieistotny
	.comp(a, b)
		porównuje a z b i zwraca warto¶æ odpowiedni±
		dla funkcji sort()

	.order
		tablica z jêzykami ustawionymi wg kolejno¶ci
		wg której maj± byæ sortowane interwiki
	.arr
		tablica z interwiki ([jêzyk, artyku³])
	.arr_i
		indeks pomocniczy, a tak¿e liczba elementów w arr
   ===================================================== */
// object init
...
/* =====================================================
	...
	.arr
		tablica z szablonami FA ([jêzyk, artyku³])
	.arr_i
		indeks pomocniczy, a tak¿e liczba elementów w arr
   ===================================================== */
// object init
...
/* =====================================================
	...
	na podgl±dzie artyku³u, w którym redirecty s± oznaczone
	specjaln± klas± (mw-redirect).

	.init()
		inicjowanie poprawek przez wstawienie ikonki przetwarzania
		wyszukanie redirectów i wys³anie wstêpnego ¿±dania
		do serwera o rozwiniêcie redirectów
	.resp(res)
		funkcja przyjmuj±ca odpowiedzieæ (res) z serwera
		i przetwarzaj±ca j± na tabelk± rozwiniêæ redirectów

	.arr	- tabela rozwiniêæ redirectów wykorzystywana wewnêtrznie
	.arr_i	- indeks u¿ywany przy tworzeniu tabeli
	.url	- url wstêpnego zapytania, potrzebny w razie
			konieczno¶ci kontynuowania ¿±dañ (wymóg API)
   ===================================================== */
//
// object init
//
...
wp_sk.redir.init = function()
{
	...
	// ograniczenie czasowe, ale tylko w podgl±dzie (¿eby nie zamêczyæ serwerów)
	if (wgAction=='submit')
	{
	...
	if (elWikiBody)
	{
		...
		{
			var $notice = jQuery('<div id="wp-sk-redir-notice">Sprawdzanie linków do przekierowañ...</div>');
...
wp_sk.redir.resp = function (jres)
{
	...
	// zbiórka t³umaczenia redirectów
	var txtescape = /([\\^\$\*\+\?\.\(\)\[\]\{\}\:\=\!\|\,\-])/g;
	...
	*/
	...
	wp_sk.cleanerLinks = function (str)
	{
		...
		return wp_sk.cleanerLinks_orig(str);	// dopiero teraz, ¿eby poprawiaæ tak¿e zmienione linki
	}
...
/* =====================================================
	...
	Dostêpna normalnie: Gecko>1.8b2
   ===================================================== */
...
jQuery( document ).ready( function() {
	...
	// kto¶ mo¿e mieæ ustawiony podgl±d od razu przy edycji - wówczas dzia³a od razu
